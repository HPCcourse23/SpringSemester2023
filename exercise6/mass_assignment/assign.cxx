...
int get_i_bin(float k, int n_bins, int nGrid, const int task = 1)
{
...
void save_binning(const int binning, std::vector<float> &fPower, std::vector<int> &nPower)
{
    ...
    ofstream f1(binning_filename);
    f1 << "P_k,k" << endl;
    for (int i = 0; i < fPower.size(); ++i)
    {
        f1 << (nPower[i] != 0 ? (fPower[i] / nPower[i]) : 0) << "," << i + 1 << endl;
    }
    f1.close();
}
...
int main(int argc, char *argv[])
{
    ...
    // Simple test
    cout << "Sum of all grid mass = " << sum(grid) << std::endl;
    ...
    vector<float> fPower(n_bins, 0.0);
    vector<int> nPower(n_bins, 0);
    float k_max = sqrt((nGrid / 2.0) * (nGrid / 2.0) * 3.0);
