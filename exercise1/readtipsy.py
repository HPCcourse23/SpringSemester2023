...
print(f"X from {min(dark['x'])} to {max(dark['x'])}")
print(f"Y from {min(dark['y'])} to {max(dark['y'])}")
print(f"Z from {min(dark['z'])} to {max(dark['z'])}")


print(f"Total mass is {sum(dark['mass'])}")
